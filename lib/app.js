...
select.addEventListener("change", () => {
  ...
  // (kleine Verbesserung): keine Listener-Kaskaden
...
applyBtn.addEventListener("click", () => {
  ...
  // Falls startCode noch nicht existiert → anlegen
  if (!codeOutput.value.includes("function startCode()")) {
    codeOutput.value =
      `function startCode() {
}
`;
  ...
  // Code in die Funktion einfügen (vor die schließende Klammer)
  codeOutput.value = codeOutput.value.replace(
    /\}\s*$/,
    "  " + preview.textContent + "\n}"
  );
});
